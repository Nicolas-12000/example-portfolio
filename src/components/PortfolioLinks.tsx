...
export default function PortfolioLinks({ dict }: { dict: any }) {
  const links = dict?.sections?.portfolioLinks || [
    { label: 'Website', href: '#' },
    { label: 'Dribbble', href: '#' },
    { label: 'Behance', href: '#' },
  ];
        ...
        {links.map((l: any, i: number) => (
          <li key={(l.href ?? '#') + '|' + (l.label ?? '') + '|' + i}>
            <Link
              ...
              target={typeof l.href === 'string' && l.href.startsWith('http') ? '_blank' : undefined}
              rel={typeof l.href === 'string' && l.href.startsWith('http') ? 'noopener noreferrer' : undefined}
            >
              {l.label}
            </Link>
          </li>
